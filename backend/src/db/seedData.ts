...
export function seedDatabase() {
  ...
  const seedParking = db.transaction(() => {
    for (const p of parkingLots) insertParking.run(p);
  });
  seedParking();
  ...
  const insertCrowdZone = db.prepare(`
  ...
  const seedCrowd = db.transaction(() => {
    for (const z of crowdZones) insertCrowdZone.run(z);
  });
  seedCrowd();
  ...
  const seedSensors = db.transaction(() => {
    for (const s of initialReadings) insertSensor.run(s);
  });
  seedSensors();
