...
function createIssuesRouter(io) {
    ...
    router.get('/', (req, res) => {
        try {
            ...
            if (status && status !== 'all') {
                ...
                const issues = database_1.db.prepare(query).all(status);
                return res.json(issues);
            }
            const issues = database_1.db.prepare(query).all();
            res.json(issues);
        }
        catch (err) {
    ...
    router.patch('/:id/status', (req, res) => {
      ...
      `);
            ...
            const updated = database_1.db.prepare('SELECT * FROM issues WHERE id = ?').get(req.params.id);
