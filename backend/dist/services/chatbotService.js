...
class ChatbotService {
     ...
     */
    static async processMessage(userMessage, sessionId = 'default-session') {
     ...
     */
    static queryBuildingOrRoom(query) {
        ...
        if (!targetBldg) {
            targetBldg = state.buildings.find((b) => lower.includes(b.code.toLowerCase()) ||
        ...
        const allRooms = state.buildings.flatMap((b) => b.rooms.map((r) => ({ ...r, building_name: b.name, building_code: b.code })));
        const matchedRoom = allRooms.find((r) => lower.includes(r.name.toLowerCase()) || r.name.toLowerCase().includes(lower));
        if (matchedRoom) {
            return {
                found: true,
                type: 'room',
     ...
     */
    static queryFreeRooms(roomType) {
        const state = dataFusion_1.DataFusionService.getFusedCampusState();
        const allFree = state.buildings.flatMap((b) => b.rooms
    ...
    static queryCrowd(query) {
        ...
        const mostCrowded = sorted[0] || { name: 'Campus Center', density_score: 50, density_level: 'moderate' };
        const quietest = sorted[sorted.length - 1] || { name: 'Quiet Gardens', density_score: 10, density_level: 'low' };
        return {
     ...
     */
    static queryIssues(location) {
        const state = dataFusion_1.DataFusionService.getFusedCampusState();
        let issues = state.issues.filter((i) => i.status !== 'resolved');
        if (location) {
            const lower = location.toLowerCase();
            issues = issues.filter((i) => i.location_name.toLowerCase().includes(lower) || i.title.toLowerCase().includes(lower));
        }
     ...
     */
    static processWithSemanticEngine(query, sessionId) {
        ...
        if (lower.includes('what do the colors mean') ||
        ...
        if (bldgOrRoomResult.found && bldgOrRoomResult.building) {
            ...
            if (lower.includes('room') || lower.includes('free') || lower.includes('lab') || lower.includes('seat') || lower.includes('vacant')) {
                ...
                if (b.free_rooms_count > 0) {
                    const sample = b.free_rooms.slice(0, 3).map((r) => `**${r.name}** (Free until ${r.next_available_time})`).join(', ');
            ...
            let buildingResp = `🏛️ **${b.name} (${b.code})**\n\n` +
                ...
                `• **Classrooms & Labs**: ${b.free_rooms_count > 0 ? `**${b.free_rooms_count} free spaces** available right now (including ${b.free_rooms.slice(0, 2).map((r) => r.name).join(', ')}).` : 'All classrooms currently occupied.'}\n`;
        ...
        if (lower.includes('free') ||
            ...
            const freeSummary = this.queryFreeRooms(lower.includes('lab') ? 'lab' : lower.includes('study') ? 'study_room' : undefined);
            const roomList = freeSummary.sample_rooms
                .map((r) => `• **${r.name}** (${r.building}, Floor ${r.floor}) — free until **${r.next_available_time}** (${r.capacity} seats)`)
                .join('\n');
            return {
        ...
        if (tokens.length > 0) {
            const matchingBldgs = state.buildings.filter((b) => tokens.some((tok) => b.name.toLowerCase().includes(tok) || b.code.toLowerCase().includes(tok) || b.description.toLowerCase().includes(tok)));
            const matchingEvents = state.events.filter((e) => tokens.some((tok) => e.title.toLowerCase().includes(tok) || e.location_name.toLowerCase().includes(tok) || e.category.toLowerCase().includes(tok)));
     ...
     */
    static async processWithClaudeLLM(userMessage, sessionId, apiKey) {
        const tools = [
            {
        ...
        if (data.stop_reason === 'tool_use') {
            ...
            if (toolCall) {
                ...
                if (followUp.ok) {
                    ...
                    const textBlock = followUpData.content.find((c) => c.type === 'text');
                    return {
